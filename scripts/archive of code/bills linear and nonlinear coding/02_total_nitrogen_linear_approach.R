...
# read in file 
decomp_n.df <- read_csv("data/cleaned/clean nitrogen.csv") %>% 
  clean_names()

# CLEAN DECOMP DATA ----

# make row a number
decomp_n.df <- decomp_n.df %>% 
...
decomp_n.df %>%
  ...
  # mutate(spp = fct_reorder2(spp, days, pct_n_remain)) %>%
...
decomp_n.df %>%
  mutate(spp = as.factor(spp)) %>% 
  mutate(spp = fct_reorder2(spp, days, pct_n_remain)) %>%
  ggplot(mapping = aes(log(days+1), log(pct_n_remain), color = interaction(spp, soil_block))) +
  # stat_summary(fun = mean, na.rm = TRUE, geom = "point") +
  # stat_summary(fun.data = mean_se, na.rm = TRUE, geom = "line") +
  geom_point()+
  geom_smooth(method="lm", se=FALSE) 
...
decomp_log.df <- decomp_n.df %>% 
  filter(days !=0 ) %>% 
  mutate(log_days = log(days+1),
         log_pct_n_remain = log(pct_n_remain))
...
k_linear.df <- decomp_log.df %>%
  ...
  mutate(
    fit = map(data, ~lm(log_pct_n_remain ~ days, data = .x))
  ) %>%
...
write_csv(k_linear_summary.df, "output/n_linear_k_values.csv")
