...
ggsave(decomp.plot, file="figures/decomposition graph.pdf",
       width=5.75, height =6.65, units="in")
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
# PC Decomp nonlin
...
pred=array(0, dim=c(5, max(pc$rep)))	
...
resid=array(0,dim=c(5, max(pc$rep)))	
#	as above

#create an array to store the observed values for making graphs:
obs=array(0,dim=c(5, max(pc$rep)))	
...
#create an array to store predicted mass remaining values:
pred=array(0, dim=c(5, max(pc_gm$rep)))	
#	array(all values = 0,dim=c(no. of rows=number of time steps, columns=number of reps)

#create an array to store the model residuals:
resid=array(0,dim=c(5, max(pc_gm$rep)))	
#	as above

#create an array to store the observed values for making graphs:
obs=array(0,dim=c(5, max(pc_gm$rep)))	
...
#create an array to store predicted mass remaining values:
pred=array(0, dim=c(5, max(cr$rep)))	
#	array(all values = 0,dim=c(no. of rows=number of time steps, columns=number of reps)

#create an array to store the model residuals:
resid=array(0,dim=c(5, max(cr$rep)))	
#	as above

#create an array to store the observed values for making graphs:
obs=array(0,dim=c(5, max(cr$rep)))	
...
#create an array to store predicted mass remaining values:
pred=array(0, dim=c(5, max(ar$rep)))	
#	array(all values = 0,dim=c(no. of rows=number of time steps, columns=number of reps)
...
k_nonlin.model = lm(k~sp, data=nonlin_k.df)
...
k_nonlin.plot <- k_nonlin.emmeans %>% 
  ggplot(aes(x=sp)) +
  geom_errorbar(aes(ymin = emmean-SE, ymax = emmean+SE), 
                stat="identity", width = 0.2, size=1) +
  geom_point(aes(y=emmean, shape=sp, color=sp, fill=sp), size=5) +
  labs(x="Species", y= "k (Proportion lost per day)")  +
  geom_text(aes(x = 1, y = .019, label = "B"),  size=6) +
  geom_text(aes(x = 2, y = .019, label = "A"),  size=6) +
  geom_text(aes(x = 3, y = .019 , label = "C"), size=6)+
  geom_text(aes(x = 4, y = .019, label = "D"),  size=6) +
...
ggsave(k_nonlin.plot, file="figures/nonlin_k_plot.pdf",
       height=7, width=7, units="in")
