...
plot_pred.df <-  with_predicted.df %>% 
  group_by(spp, soil_block, days) %>% 
  summarize(mean_loss = mean(pct_mass_remain, na.rm=TRUE),
            mean_pred = mean(.fitted, na.rm=TRUE))

plot_pred.df %>% 
  ggplot(aes(x= days, color=spp, shape=as.factor(soil_block))) +
